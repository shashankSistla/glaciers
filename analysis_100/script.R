...
for(glacier in glacier_list){
  ...
  for(path in selected_paths){
    ...
    plot_name =  paste0(key,glacier,"_",path,"_original vs fit.png")
    png(plot_name)
    ...
    saveRDS(list(R_squared = R_squared, params_estimated = params_estimated, results = results), file = paste0(glacier,"_",path,"_results.rds"))
    plot_name =  paste0(key,glacier,"_",path,"_simulated.png")
    png(plot_name)
    plot_name = paste(key,glacier,'-',R_squared)
    ...
    plot_name =  paste0(key,glacier,"_",path,"_residuals.png")
    png(plot_name)
    # Plot the residuals
