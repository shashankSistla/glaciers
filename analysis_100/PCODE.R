...
innerobj <- function(basis_coef, ode.par, input, derive.model, NLS = TRUE) {
  ...
  xt_time_pairs = list()
  for (i in 1:length(quadts)) {
    xt_time_pairs[[i]] = list(x = Xt[i], t = quadts[i])
  }
  tempfun <- function(pair, parameters) {
    x = pair$x
    t = pair$t
    return(derive.model(t, state = c(X = x), parameters = parameters))
  }
  
  rightside <- unlist(lapply(xt_time_pairs, tempfun, parameters = ode.par))
